% Demo script: Test LSPI solution of infinite horizon linear-quadratic regulator problem in discrete time.
...
p          = [];
model.phi  = @(z)phi_lqrq(z(1:dimX,:),z(dimX+1:end,:)); % basis functions
argmaxuQ   = @(theta)argmaxuQ_phi_lqrq(theta); % function for finding the argmax of the Q-function
pi0        = @(x)pi_linear(x,[1,1]);        % initial policy

% optimise
[pi, Qp, model] = lspi( D, pi0, model, argmaxuQ, gamma, p );
...
[dummy,L]= argmaxuQ(model.w); % get gains (for evaluating errors)
...
Nu=min(size(u,2),size(ut,2)); RMSE_u=sqrt(sum((ut(:,1:Nu)-u(:,1:Nu)).^2,2)/Nu);
                              RMSE_L=norm(L-Lt);
fprintf(1,'RMSE(x,x*)=[%f %f],RMSE(u,u*)=%f,||L-L*||=%f\n',RMSE_x,RMSE_u,RMSE_L)

% plot
fn=1;figure(fn),clf
plot_lq_fh(fn,'LSPI/LQR-IH (discrete time)',t,x,u,[],[],'b');
