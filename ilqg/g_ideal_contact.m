...
%
function [xdot, xdot_x, xdot_u] = g_ideal_contact ( x, u, model )

xdot = f_ideal_contact ( x, u, model );
...
if nargout > 1
	...
	fx=@(x)qddot_ideal_contact(x(1),x(2),u,model);
	dqddotdx=J_fd(fx,x);
	fu=@(u)qddot_ideal_contact(x(1),x(2),u,model);
	dqddotdu=J_fd(fu,u);

	xdot_x = [0 1; dqddotdx];
	xdot_u = [0 0 0; dqddotdu];
