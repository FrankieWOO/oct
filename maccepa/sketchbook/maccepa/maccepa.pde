 ...
 *
 * This is initiated when Atmel Timer 2 is reset, which is 4x as often as the
 * Timer 1 interrupt.
 * This means that in one control cycle, we read the sensors 4 times.
 * We can exploit this for noise reduction,
 * by accumulating the readings and taking their average.
 * This is automatically done when using regression for the system identification.
 */
ISR(TIMER2_OVF_vect) {
 ...
 *
 * This is initiated by the USART Tx Complete Interrupt, triggered by setting
 * TXCIE0 bit in UCSR0B
 *
...
/**
 * \brief Main loop. Listens for serial communications,
 * copies these to the command buffer when a full command has been recieved.
 */
void loop() {
